...
import type { DieselExpense } from '@/types/database'
import {
  normalizePlate,
  normalizePoNumber,
  resolveAmount,
...
function validateDiesel(form: Record<string, string>): string | null {
...
export function DieselPage() {
  const { plateOptions, supplierOptions } = useMasterData()

  const columns = [
    { key: 'date' as const, label: 'Date', inputType: 'date' as const, required: true },
    { key: 'po_number' as const, label: 'P.O Number' },
    {
  ...
  return (
    <TransactionModule<DieselExpense>
